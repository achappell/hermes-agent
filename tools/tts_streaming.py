...
def resolve_streaming_provider(
    ...
    ``auto`` returns the first usable in ``_PROVIDER_PRIORITY``; ``none``/``off``/``disabled``/
    ``false`` disables streaming outright (no fallthrough to the configured provider — lets a
    profile choose the complete route when the chunked route has less consistent prosody).
    Otherwise the configured TTS provider (or ``preferred``): ``None`` means "no chunked API" —
    the dispatcher speaks per-sentence via the sync path, preserving the user's chosen voice. We
    never silently swap providers just to get streaming."""
